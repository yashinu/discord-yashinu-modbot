...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!client.kullanabilir(message.author.id) && !message.member.roles.cache.has(ayar.sahipRolu) && !ayar.banciRolleri.some(r => message.member.roles.cache.has(r)) && !message.member.roles.cache.has("721069802793467935")) return message.channel.send(embed.setDescription("Bu komutu kullanabilmek için gerekli rollere sahip değilsin!")).then(x => x.delete({timeout: 5000}));
  ...
  let teyitciler = ayar.teyitciRolleri || [];
  if (teyitciler.some(c => victim.roles.cache.has(c))) return message.reply(`Yetkililerden herhangi birini banlayamazsin!`);
  ...
  message.channel.send(embed.setImage(`https://cdn.discordapp.com/attachments/722850373412454461/724973805566689370/20200613_201623.gif`).setDescription(`\`${victim.user.tag}\` üyesi ${message.author} tarafından **${reason}** nedeniyle **banlandı!**`));
