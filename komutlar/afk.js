...
module.exports.execute = (client, message, args, ayar, emoji) => {
  if (!message.member.roles.cache.has(ayar.ekipRolu)) return message.reply(`Bu özellik sadece taglı üyelerimiz içindir. Tag alıp kullanabilirsin.\n ${ayar.tag}`);
  let sebep = args.join(' ');
  ...
  if (message.member.manageable && (message.member.nickname && !message.member.nickname.includes(`[AFK]`))) message.member.setNickname(`[AFK]${message.member.displayName}`).catch(console.log);
