...
module.exports.execute = async (client, message, args) => {
  ...
  let ozellikler = [
    ...
    { name: "uyariLogKanali", type: "kanal" },
    ...
    { name: "sesMuteLogKanali", type: "kanal" },
    { name: "sesMuteciRolleri", type: "roller" },
    { name: "jailRolu", type: "rol" },
    { name: "jailciRolleri", type: "roller" },
    { name: "jailLogKanali", type: "kanal" }
  ];
  let secim = args[0];
  const embed = new MessageEmbed().setColor(client.randomColor()).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setFooter(conf.durum);
  if (secim == "liste") {
  ...
  if (secim === "sıfırla") {
    if (!args[1] || (args[1] !== "teyit")) return message.channel.send(embed.setDescription("Sıfırlanacak veriyi belirtmelisin! (teyit)")).then(x => x.delete({timeout: 5000}));
    if (args[1] === "teyit") {
      kdb.delete("teyit");
    };
