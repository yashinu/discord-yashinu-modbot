...
module.exports = async (member) => {
  ...
  let seslimute = jdb.get("seslimute") || [{id: null}];
  ...
  if(jaildekiler.some(x => x.includes(member.id)) || tempjaildekiler.some(x => x.id === member.id)){
    ...
    if(ayarlar.jailRolu) member.roles.set([ayarlar.jailRolu]).catch();
    if(ayarlar.jailLogKanali && member.guild.channels.cache.has(ayarlar.jailLogKanali)) return member.guild.channels.cache.get(ayarlar.teyitKanali).send(new MessageEmbed().setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setDescription(`${member} üyesi sunucuya katıldı fakat hesabı ${member.client.tarihHesapla(member.user.createdAt)} açıldığı için jaile atıldı!`).setTimestamp().setFooter("YASHINU ❤️ ALOSHA"));
